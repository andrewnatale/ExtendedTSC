...
datfile_dir = '/Users/anatale/school/UCSF/Grabe_lab/data/traj_features/50ps/pore_features'

traj_names = ['traakWT_full_npt',
              'traakWT_S1S3_npt',
              'traakWT_S2S4_npt',
              'traakG124I_full_npt',
              'traakG124I_S1S3_npt',
...
feature_names = ['pore_bb_dihedrals',
                 'pore_carbonyls',
                 'pore_potassium',
                 'pore_water']

figures_dir = '/Users/anatale/school/UCSF/Grabe_lab/data/traj_features/50ps/pore_features/figures'
...
def dihedral_vis(save=False):
    ...
    for key1 in sorted(datasets.keys()):
        ...
        ax.set_title(key1)
        if save:
            f.savefig(os.path.join(figures_dir,'%s.png' % key1), bb_inches='tight')
        else:
            plt.show()
...
def vectorize(key):
    ...
    vectorDS.add_measurement(('pore_K','boolean','K+ in site below S4'), width=1)
    vectorDS.add_measurement(('s4_K','boolean','K+ in S4'), width=1)
    vectorDS.add_measurement(('s3_K','boolean','K+ in S3'), width=1)
    vectorDS.add_measurement(('s2_K','boolean','K+ in S2'), width=1)
    vectorDS.add_measurement(('s1_K','boolean','K+ in S1'), width=1)
    vectorDS.add_measurement(('s0_K','boolean','K+ in S0'), width=1)
    ...
    tsc_writer._data_writer(vectorDS,outfilename='%s_vectorized.dat' % key)
...
def examine_vectors():
    vectors = {}
    for elem in traj_names:
        vectors[elem] = tsc_writer._data_reader('%s_vectorized.dat' % elem, True)
    # print vectors.values()
    for key in vectors:
        potassium = np.sum(vectors[key].data[1:5,:], axis=0)
        water = np.sum(vectors)
#for key in vectors
examine_vectors()
